...
def _generate_weights_pie_chart(weights_dict, output_path):
    ...
    try:
        ...
        ax.axis('equal')
        # Đã xóa 2 dòng tạo lỗ hổng ở giữa
...
def _generate_ranking_map(ranking_df, geojson_path, output_path):
    """
    Tạo bản đồ tĩnh GeoJSON
    *** CẬP NHẬT: Tô màu theo Điểm TOPSIS (0-1) và dùng gam màu Xanh (Greens) ***
    """
    try:
        ...
        merged_gdf.plot(
            ...
            ax=ax,
            legend=True,
            cmap='Greens',  # Gam màu xanh (càng xanh đậm càng tốt)
...
def create_full_report(model_name: str, all_weights: dict) -> str | None:
    ...
    try:
        ...
        return None# report_module.py (Phiên bản PDF Nâng cấp)
"""
Module để tạo báo cáo PDF tĩnh, được thiết kế lại
để trông chuyên nghiệp hơn, lấy cảm hứng từ mẫu HTML.
...
class PDF(FPDF):
    ...
    def section_title(self, title):
        self.set_font(self.active_font, 'B', 14)
        # Dùng màu cam cho tiêu đề
        self.set_fill_color(int(PALETTE['orange'][1:3], 16), int(PALETTE['orange'][3:5], 16), int(PALETTE['orange'][5:7], 16))
        self.set_text_color(255, 255, 255)
        self.cell(0, 10, f' {title}', 0, 1, 'L', fill=True)
        self.set_text_color(0, 0, 0)
        self.ln(5)
